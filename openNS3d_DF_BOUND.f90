...
subroutine OCFD_DF1_BOUND_5th(v,df,n,h,flag)
    ...
    S0=13.d0/12.d0*(v(2)-2.d0*v(3)+v(4))**2+   &
    ...
    S0=13.d0/12.d0*(v(n-2)-2.d0*v(n-1)+v(n))**2+   &
        1.d0/4.d0*(3.d0*v(n-2)-4.d0*v(n-1)+v(n))**2

    S1=13.d0/12.d0*(v(n-3)-2.d0*v(n-2)+v(n-1))**2+   &
        1.d0/4.d0*(v(n-3)-v(n-1))**2

    S2=13.d0/12.d0*(v(n-4)-2.d0*v(n-3)+v(n-2))**2+   &
        1.d0/4.d0*(v(n-4)-4.d0*v(n-3)+3.d0*v(n-2))**2
	...
	q03=1.d0/3.d0*v(n-2)+5.d0/6.d0*v(n-1)-1.d0/6.d0*v(n)
	q13=-1.d0/6.d0*v(n-3)+5.d0/6.d0*v(n-2)+1.d0/3.d0*v(n-1)
	q23=1.d0/3.d0*v(n-4)-7.d0/6.d0*v(n-3)+11.d0/6.d0*v(n-2)
...
!		f(1)=0.d0
!		f(2)=0.d0
		f(1)=(-25.d0/12.d0*u(1)+4.d0*u(2)-3.d0*u(3)+4.d0/3.d0*u(4)-u(5)/4.d0)/h
		f(2)=(-u(1)/4.d0-5.d0/6.d0*u(2)+1.5d0*u(3)-0.5d0*u(4)+u(5)/12.d0)/h
	elseif (flag.eq.2) then
...
!		f(n)=0.d0
!		f(n-1)=0.d0
		f(n)=(25.d0/12.d0*u(n)-4.d0*u(n-1)+3.d0*u(n-2)-4.d0/3.d0*u(n-3)+u(n-4)/4.d0)/h
		f(n-1)=(u(n)/4.d0+5.d0/6.d0*u(n-1)-1.5d0*u(n-2)+0.5d0*u(n-3)-u(n-4)/12.d0)/h
	endif
end subroutine
...
!		s(1)=0.d0
		s(1)=(15.d0/4.d0*u(1)-77.d0/6.d0*u(2)+107.d0/6.d0*u(3) &
			-13.d0*u(4)+61.d0/12.d0*u(5)-5.d0/6.d0*u(6))/h2
	elseif (flag.eq.2) then
!		s(n)=-4.d0*pi2*dsin(2.d0*pi*1.d0)-4.d0*pi2*dcos(2.d0*pi*1.d0)
!		s(n)=(4.d0*(10.d0-5)**2-2.d0)*exp(-(10.d0-5.d0)**2)
!		s(n)=0.d0
		s(n)=(15.d0/4.d0*u(n)-77.d0/6.d0*u(n-1)+107.d0/6.d0*u(n-2) &
			-13.d0*u(n-3)+61.d0/12.d0*u(n-4)-5.d0/6.d0*u(n-5))/h2
	endif
