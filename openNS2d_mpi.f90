...
subroutine check_xbound(f)
	include 'openNS3d.h'
		
	integer i,j
	real(kind=OCFD_REAL_KIND):: f(1-LAP:nx+LAP,1-LAP:ny+LAP),   &
		    tmp_send1(ny), tmp_recv1(ny)
	if (npx0==1) then	  
		f(nx,:)=f(1,:)
	else 
		if (npx .eq. 0) then	
			do j=1,ny
				tmp_send1(j)=f(1,j)
			enddo
		call MPI_Send(tmp_send1,ny,OCFD_DATA_TYPE,ID_XM1,9001,MPI_COMM_WORLD,Status,ierr)
    	endif
		if (npx .eq. npx0-1) then	
		call MPI_Recv(tmp_recv1,ny,OCFD_DATA_TYPE,ID_XP1,9001,MPI_COMM_WORLD,Status,ierr)
		do j=1,ny
			f(nx,j)=tmp_recv1(j)
		enddo
    	endif	
